%MCEN90018 - Advanced Fluid Dynamics - Assignment 2 - Q5
% This script answers Question 5 in the Assignment 2 sheet.
...
contourf(xM, yM, uHat, 25, 'LineColor', 'None')
...
for i = 1:size(H,2)
    ...
    if i == size(H,2)
        plot(streamline(:,1),streamline(:,2),'-r', 'LineWidth',2);
    end
end
...
Uc = Uc*0.5; %not clear why this fixes everything in the numerical vs theory calculation. Can't see where the additional factor 2x is in the getUi, etc code
Cp = 1 - (Uc./Uinf).^2;
...
title("Comparison of theoretical vs numerical C_p for vortex 64-panel geometry")
grid on
axis([0 2*pi -3 1]);
...
function plotCircleGeometry(x,y,colourString)
    ...
    daspect([1 1 1]);
    title(['Flow around circle geometry with ' num2str(size(x,1)) ' vortex panels']);
    xlabel('x')
    ylabel('y')
end
